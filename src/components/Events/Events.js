...
const Events = () => {
  ...
  const result = useMemo(() => {
    if (fil) {
     return events.filter(item => (item.title.toLowerCase()).includes(fil.toLowerCase()));
      ...
      return events.filter(item => (item.category.toLowerCase() === category.toLowerCase()));
    }
    else {
      return events;
    }
  ...
  return (
    <div >
       <Buttons/>
       {fil || category ? (<button className='goback' onClick={resetFilters}>Back to all events</button>) : null}
